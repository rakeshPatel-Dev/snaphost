    ...
    <div className="rounded-xl border border-border bg-card p-4 shadow-sm">
      <div className="flex items-start justify-between gap-3">
        <div className="min-w-0 flex-1">
          <div className="flex items-center gap-2 min-w-0">
            <div className="flex h-9 w-9 shrink-0 items-center justify-center rounded-lg bg-muted/20 border border-border">
              <span className="text-[10px] font-bold text-foreground">{fileType === 'pdf' ? 'PDF' : 'IMG'}</span>
            </div>
        ...
        <div className="flex shrink-0 flex-col gap-2">
          <Button size="sm" variant="outline" className="h-8 gap-1.5" onClick={() => onCopy(url, id)}>
            {copied ? <Check className="h-3.5 w-3.5 text-emerald-500" /> : <Copy className="h-3.5 w-3.5" />}
            {copied ? 'Copied' : 'Copy'}
          </Button>
          <Button size="sm" variant="outline" className="h-8 gap-1.5" onClick={() => onOpen(url)}>
