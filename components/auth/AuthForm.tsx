...
import { BrandDiscord, BrandGithub, BrandGoogle } from '@/components/icons/brand-icons'
...
const OAUTH_ICONS: Record<'google' | 'github' | 'discord', React.ReactNode> = {
  google: <BrandGoogle className="size-4" />,
  github: <BrandGithub className="size-4" />,
  discord: <BrandDiscord className="size-4" />,
}
...
export default function AuthForm({ mode }: AuthFormProps) {
  ...
  async function handleOAuth(provider: 'google' | 'github' | 'discord') {
    ...
    if (error) {
      ...
      toast.error(getAuthErrorMessage(error, AUTH_ERRORS.oauthFailed))
      setLoading(false)
    }
  }
    ...
    <div className="w-full rounded-4xl border border-border/60 bg-card/80 p-7 backdrop-blur-xl shadow-[0_30px_80px_-40px_rgba(0,0,0,0.18)] sm:p-8">
      ...
      <form className="space-y-4" onSubmit={handleEmailAuth}>
        ...
        <Button
          ...
          disabled={
            loading ||
            (!isSignIn && (!isUsernameAvailable || isUsernameChecking || !passwordIsStrong))
          }
        >
